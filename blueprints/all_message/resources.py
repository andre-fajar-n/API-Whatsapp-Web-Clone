...
class AllMessageResource(Resource):
    ...
    def get(self):
        ...
        for conversation in conversations:
            ...
            for personal_message in personal_messages:
                ...
                if marshal_personal_message['user_id'] == marshal_conversation['info_chat']['id']:
                    user = Users.query.get(marshal_conversation['info_chat']['id'])
                    marshal_user = marshal(user, Users.response_fields)
        ...
        for group in group_by_user:
            ...
            chat_groups = MessageGroup.query.filter_by(group_chat_id=group.group_chat_id)
            
            # memasukkan data chat terakhir
            last_chat_group = chat_groups.order_by(desc(MessageGroup.created_at)).first()
            marshal_last_chat_group = marshal(last_chat_group, MessageGroup.response_fields)
            marshal_group['last_chat'] = marshal_last_chat_group
            
            # memasukkan semua chat
            all_chat_group = []
            for chat_group in chat_groups:
                marshal_chat_group = marshal(chat_group, MessageGroup.response_fields)
            ...
            for index1 in range(len(list_chat)-1):
                for index2 in range(0, len(list_chat) - index1 - 1):
                    ...
                    earlier = earlier[slice(5,len(earlier))]
                    earlier = earlier.split()
                    print(earlier)
                    print(moment.date('12-18-2020'))
                    
                    # if list_chat[index2]['last_chat']['created_at'] > list_chat[index2 + 1]['last_chat']['created_at']:
                        #     earlier = list_chat[index2 + 1]['last_chat']['created_at']
                    #     more_late = list_chat[index2]['last_chat']['created_at']
                    # list_chat[index2], list_chat[index2 + 1] = earlier, more_late
