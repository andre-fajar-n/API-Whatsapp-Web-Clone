...
class PersonalMessageResource(Resource):
    ...
    def get(self):
        parser = reqparse.RequestParser()
        parser.add_argument('user2_id', location='args', type=int)
        args = parser.parse_args()
        ...
        for personal_message in personal_messages:
            ...
            if marshal_personal_message['user_id'] == args['user2_id']:
                user = Users.query.get(args['user2_id'])
                marshal_user = marshal(user, Users.response_fields)
                marshal_personal_message['user'] = marshal_user
                
            content_of_messages.append(marshal_personal_message)
            
        marshal_conversation['conversation'] = content_of_messages
        
        app.logger.debug('DEBUG: %s', marshal_conversation)
        return marshal_conversation, 200
